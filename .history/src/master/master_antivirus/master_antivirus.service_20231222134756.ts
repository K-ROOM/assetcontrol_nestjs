...
export class MasterAntivirusService {
  ...
  findAllCount() {
    ...
    SELECT COUNT(*) AS antiviruscount FROM tblMaster_Antivirus`);
  }

  findOne(antivirusVersion: string) {
    return this.masterAntivirusRepository.findOne({
      where: { antivirusVersion: antivirusVersion },
    });
  }
