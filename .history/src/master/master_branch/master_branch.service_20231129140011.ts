...
export class MasterBranchService {
  ...
  findAllSelectCount() {
    ...
    (SELECT COUNT(Status) AS ActiveX FROM tblIPAddress_Status WHERE (BranchCode = tblMaster_Branch.BranchCode) AND (Status = 'Active')) AS Active,
    (SELECT COUNT(Status) AS EmptyX FROM tblIPAddress_Status AS tblIPAddress_Status_2 WHERE (BranchCode = tblMaster_Branch.BranchCode) AND (Status = 'Empty')) AS Empty
