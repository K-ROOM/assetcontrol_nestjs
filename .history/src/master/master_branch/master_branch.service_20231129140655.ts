...
export class MasterBranchService {
  ...
  (SELECT COUNT(Status) AS Active FROM tblIPAddress_Status WHERE (BranchCode = tblMaster_Branch.BranchCode) AND (Status = 'Active')) AS Status_Active,
  (SELECT COUNT(Status) AS Empty FROM tblIPAddress_Status AS tblIPAddress_Status_2 WHERE (BranchCode = tblMaster_Branch.BranchCode) AND (Status = 'Empty')) AS Status_Empty
