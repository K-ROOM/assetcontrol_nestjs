  ...
  async createHistoryTransaction(data: any) {
    const query = `
        ...
        FROM 
            tblAssetMain AS A1 
        INNER JOIN 
            tblMaster_SubCategory AS A2 
        ON 
            A1.SubCategory = A2.SubCategory
        WHERE 
            (A2.AnnualCheck = 1) 
            AND 
            (A1.AnnualCheckStatus IN ('Ok', 'Wait')) 
            AND 
            (A1.Status IN ('Active', 'In Stock'));
    `;

    return this.checkPeriodRepository.query(query, [data.halfName, data.workYear]);
  }
