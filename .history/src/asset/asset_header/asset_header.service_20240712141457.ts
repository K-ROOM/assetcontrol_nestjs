...
export class AssetHeaderService {
  ...
  findAllCount() {
    ...
    (SELECT COUNT(*) FROM tblAssetMain WHERE Category = 'Computer' AND Status = 'Active') AS ComputerActiveCount,
	  (SELECT COUNT(*) FROM tblAssetMain WHERE Category = 'Computer' AND Status = 'In Stock') AS ComputeInStockCount,
	...
	(SELECT COUNT(*) AS TotalAsset FROM (SELECT Status FROM tblAssetMain UNION ALL SELECT Status FROM tblSpareParts) AS combinedTotal) AS TotalAsset`);
  }
