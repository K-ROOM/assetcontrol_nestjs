...
export class AssetHeaderService {
  ...
  async findAllSelectCol(category: string, branchCode: string) {
    const assetRepository = assetRepository(Asset);
    
    return await assetRepository.createQueryBuilder('asset')
      .select([
        'asset.edp_No', 'asset.fin_No', 'asset.category', 'asset.subCategory',
        'asset.status', 'asset.firstCheck', 'asset.lastCheck', 'asset.inputUser',
        'asset.modifiedBy', 'asset.branchCode', 'asset.userName', 'asset.annualCheckStatus'
      ])
      .where('asset.category = :category', { category })
      .andWhere('asset.branchCode = :branchCode', { branchCode })
      .orderBy('asset.branchCode', 'ASC')
      .getMany();
  }
